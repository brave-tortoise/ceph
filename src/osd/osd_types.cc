...
ostream& operator<<(ostream& out, const pg_log_entry_t& e)
{
  out << e.version << " (" << e.prior_version << ") "
      ...
      << "delta_recovery = " << e.delta_recovery << " dirty_regions: " << e.dirty_regions;
  if (e.snaps.length()) {
...
{
  if (e.is_update()) {
    map<hobject_t, item>::iterator it = missing.find(e.soid);
    bool is_missing_divergent_item = (it != missing.end());
	...
	//missing[e.soid].merge(e);
	rmissing.erase(it->second.need.version);
	it->second.merge(e);
	it->second.have = eversion_t();
      } else {
...
void pg_missing_t::revise_need(hobject_t oid, eversion_t need)
{
  if (missing.count(oid)) {
    ...
    missing[oid].delta_recovery = false;
  } else {
...
void pg_missing_t::revise_have(hobject_t oid, eversion_t have)
{
  if (missing.count(oid)) {
    ...
    missing[oid].delta_recovery = false;
    missing[oid].dirty_regions.clear();
  }
}
