...
public:
  ...
  */
  ...
  Mutex flush_lock;
  Cond flush_cond;
  FIFOCache<hobject_t, FlushInfo> flush_queue;
  int flush_ops;
  unordered_set<hobject_t> flush_oids;
  struct FlushThread : public Thread {
    OSDService *osd;
    FlushThread(OSDService *o) : osd(o) {}
    void *entry() {
      osd->flush_entry();
      return NULL;
    }
  } flush_thread;
  bool flush_stop_flag;

  void flush_entry();
  void flush_stop();

  /// insert an object into promote_queue
  bool flush_enqueue_object(const hobject_t& oid, const FlushInfo& info) {
    Mutex::Locker l(flush_lock);
    if(!flush_oids.count(oid)) {
      flush_queue.add(oid, info);
      flush_cond.Signal();
      return true;
    }
    return false;
  ...
  void flush_dequeue_object(const hobject_t& oid) {
    Mutex::Locker l(flush_lock);
    flush_queue.remove(oid);
  }

  /// note start of an async (flush) op
  bool flush_start_op(const hobject_t& oid) {
    Mutex::Locker l(flush_lock);
    if(!flush_oids.count(oid)) {
      ++flush_ops;
      flush_oids.insert(oid);
      return true;
    }
    return false;
  }
  ...
  /// note finish or cancellation of an async (flush) op
  void flush_finish_op(const hobject_t& oid) {
    Mutex::Locker l(flush_lock);
    assert(flush_ops > 0);
    --flush_ops;
    assert(flush_oids.count(oid) == 1);
    flush_oids.erase(oid);
    flush_cond.Signal();
  }

  /// get count of active flush ops
  int flush_get_num_ops() {
    Mutex::Locker l(flush_lock);
    return flush_ops;
  }
  */
